...
from gramps.gen.plug._pluginreg import PluginRegister, PTYPE, REPORT, REPORT_MODE_GUI, REPORT_MODE_BKI
...
config = configman.register_manager("report-launcher")
config.register("defaults.reportlist", [])
...
class Launcher(Gramplet):
    ...
    def populate_linkframe(self):
        ...
        config.set("defaults.reportlist", toollist)
        config.save()
    ...
    def populate_pluginsframe(self):
        ...
        for ptype, pd in all_plugins():
            if ptype == REPORT and REPORT_MODE_GUI in pd.report_modes:
                catname = standalone_categories[pd.category][1]
        ...
        config.load()
        toollist = config.get("defaults.reportlist")
    ...
    def run_tool(self, widget, event):
        pdata = widget.pdata
        run_plugin(pdata, self.dbstate, self.uistate)
